...
let push_node stack pred v = 
  if (Stack.length stack) = 0 then
    Stack.push [(pred, v)] stack
  else
    Stack.push ((pred, v) :: (Stack.top stack)) stack
...
let compute_paths max_depth g v =
  ...
  in
    (* visit 0 v; *)
    printf "compute_paths start %s outdeg %d indeg %d\n" v (G.out_degree g v) (G.in_degree g v);
...
let neighbourhood g u h =
  let neigh = ref [] in
  let visit v d =
    (* printf "n5_visit %s depth %d\n" v d; *)
    if d <= h then
      neigh := v :: !neigh
  in
    printf "n5 neighbourhood %s\n" u;
    bfs_dist_iterator visit g u;
    printf "n5 neighbourhood size %d\n" (List.length !neigh);
    !neigh
...
let redundancy_worker g (vs : G.V.t list) benchmark : result list =
  let single v =
    benchmark ();
...
module Redundancy_job : Mpi_framework.Mpi_job = struct
  include G
  type worker_result = result list
  let worker_function = redundancy_worker
  type combine_type = result list
  let combine_start = []
